...
float * iterativeCpu(const float * initX, const float * rhs,
                  ...
                  const float * rightMatrix, const float * topMatrix, 
		  const float * bottomMatrix, int nxGrids, int nyGrids,
			 ...
			 int nxGrids, int nyGrids, int nIters, const int threadsPerBlock, int method)
{
    ...
    float * rhsGpu, * leftMatrixGpu, * rightMatrixGpu, * centerMatrixGpu, * topMatrixGpu, * bottomMatrixGpu;
    ...
    cudaMemcpy(leftMatrixGpu, leftMatrix, sizeof(float) * nDofs,
            cudaMemcpyHostToDevice);
    cudaMemcpy(centerMatrixGpu, centerMatrix, sizeof(float) * nDofs,
            cudaMemcpyHostToDevice);
    cudaMemcpy(rightMatrixGpu, rightMatrix, sizeof(float) * nDofs,
            cudaMemcpyHostToDevice);
    cudaMemcpy(topMatrixGpu, topMatrix, sizeof(float) * nDofs,
            cudaMemcpyHostToDevice);
    cudaMemcpy(bottomMatrixGpu, bottomMatrix, sizeof(float) * nDofs,
            cudaMemcpyHostToDevice);
...
void __iterativeBlockUpdateToNorthSouth(float * xTopBlock, float * xBottomBlock, const float *rhsBlock, 
                             const float * leftMatrixBlock, const float *centerMatrixBlock, const float * rightMatrixBlock, 
			     const float * topMatrixBlock, const float * bottomMatrixBlock, int nxGrids, int nyGrids, int iGrid, int method, int subdomainLength, bool vertical, int maxSteps)
{
    extern __shared__ float sharedMemory[];
    float * x0 = sharedMemory; 
    int elemPerBlock = subdomainLength * subdomainLength;
...
void _iterativeGpuOriginal(float * xLeftGpu, float *xRightGpu,
			     ...
			     const float * topMatrixGpu, const float * bottomMatrixGpu, int nxGrids, int nyGrids, int method, int subdomainLength, int maxSteps)
{
...
{     
    ...
    float * x0Gpu, * rhsGpu, * leftMatrixGpu, * rightMatrixGpu, * centerMatrixGpu, * topMatrixGpu, * bottomMatrixGpu;
    cudaMalloc(&x0Gpu, sizeof(float) * nDofs);
    cudaMalloc(&rhsGpu, sizeof(float) * nDofs);
    cudaMalloc(&leftMatrixGpu, sizeof(float) * nDofs);
    cudaMalloc(&centerMatrixGpu, sizeof(float) * nDofs);
    ...
    for (int i = 0; i < nIters; i++) {
        ...
        _iterativeGpuOriginal <<<grid, block, sharedBytes>>>(xLeftGpu, xRightGpu, x0Gpu, rhsGpu, leftMatrixGpu, centerMatrixGpu, rightMatrixGpu, topMatrixGpu, bottomMatrixGpu, nxGrids, nyGrids, method, subdomainLength, maxSteps);

        // APPLY HORIZONTAL SHIFT
        _iterativeGpuHorizontalShift <<<grid, block, sharedBytes>>> (xLeftGpu, xRightGpu, xTopGpu, xBottomGpu, x0Gpu, rhsGpu, leftMatrixGpu, centerMatrixGpu, rightMatrixGpu, topMatrixGpu, bottomMatrixGpu, nxGrids, nyGrids, method, subdomainLength, maxSteps);

        // APPLY VERTICAL SHIFT (ALONG WITH PREVIOUS HORIZONTAL SHIFT)
        _iterativeGpuVerticalandHorizontalShift <<<grid, block, sharedBytes>>> (xLeftGpu, xRightGpu, xTopGpu, xBottomGpu, x0Gpu, rhsGpu, leftMatrixGpu, centerMatrixGpu, rightMatrixGpu, topMatrixGpu, bottomMatrixGpu, nxGrids, nyGrids, method, subdomainLength, maxSteps);

        // APPLY VERTICAL SHIFT
        _iterativeGpuVerticalShift <<<grid, block, sharedBytes>>> (xLeftGpu, xRightGpu, xTopGpu, xBottomGpu, x0Gpu, rhsGpu, leftMatrixGpu, centerMatrixGpu, rightMatrixGpu, topMatrixGpu, bottomMatrixGpu, nxGrids, nyGrids, method, subdomainLength, maxSteps);
...
int main(int argc, char *argv[])
{
    ...
    float * leftMatrix = new float[nDofs];
    float * centerMatrix = new float[nDofs];
    float * rightMatrix = new float[nDofs];
    float * bottomMatrix = new float[nDofs];
    float * topMatrix = new float[nDofs];
    ...
    float * solutionGpuClassic = iterativeGpuClassic(initX, rhs, leftMatrix, centerMatrix,
    ...
    float * solutionGpuSwept = iterativeGpuSwept(initX, rhs, leftMatrix, centerMatrix,
                                                  rightMatrix, topMatrix, bottomMatrix, nxGrids, nyGrids, nCycles, maxSteps, threadsPerBlock, method, subdomainLength);
...
*/
    ...
    float residualCPU = Residual(solutionGpuClassic, rhs, leftMatrix, centerMatrix, rightMatrix, topMatrix, bottomMatrix, nxGrids, nyGrids);
    float residualClassicGPU = Residual(solutionGpuClassic, rhs, leftMatrix, centerMatrix, rightMatrix, topMatrix, bottomMatrix, nxGrids, nyGrids);
    float residualSweptGPU = Residual(solutionGpuSwept, rhs, leftMatrix, centerMatrix, rightMatrix, topMatrix, bottomMatrix, nxGrids, nyGrids);
