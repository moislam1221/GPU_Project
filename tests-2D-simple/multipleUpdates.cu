...
int main()
{
    ...
    for (int iGrid = 0; iGrid < nDofs; iGrid++) 
    {
        ...
        bottomMatrixCpu[iGrid] = -1.0 / (dx * dx);
        topMatrixCpu[iGrid] = -1.0 / (dx * dx);
        rhsCpu[iGrid] = 1.0;
    }
    ...
    // PRINT TO SCREEN
    std::cout << "\n" << "NOW COMPUTING RESIDUAL OF SOLUTION" << "\n" << std::endl;
